...
if st.session_state.df_merged is not None:
    ...
    # --- 5. HİSSE TEŞHİS PANELİ ---
        ...
        # Anlık arama özellikli selectbox
                    ...
                    try:
                        ...
                        if not h_yf.empty:
                            ...
                            if len(cls) >= 200:
                               ...
                               # --- YERLEŞİK GRAFİK (Kütüphane gerektirmez) ---
                h_yf = yf.download(secilen_hisse + ".IS", period="1y", progress=False)
                if not h_yf.empty:
                    h_yf['MA20'] = h_yf['Close'].rolling(20).mean()
                    h_yf['MA75'] = h_yf['Close'].rolling(75).mean()
                    # Sadece Fiyat ve MA'ları çizdiriyoruz (otomatik interaktiftir)
                    st.line_chart(h_yf[['Close', 'MA20', 'MA75']])
                    st.info("Teknik trendi yukarıdaki grafik üzerinden inceleyebilirsin.")
